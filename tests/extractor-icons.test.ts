...
const nextRoot = () =>
  path.join(os.tmpdir(), `ui-harvester-icon-tests-${Date.now()}-${Math.round(Math.random() * 1_000_000)}`);
...
describe('icon harvesting', () => {
  ...
  it('normalizes icon URLs and keeps clean filenames', async () => {
    ...
    expect(extractor.canonicalizeIconSourceUrl('/icons/favicon.ico', 'https://example.test/page')).toBe(
      'https://example.test/icons/favicon.ico'
    );
    expect(
      extractor.canonicalizeIconSourceUrl('https://example.test/icon.svg?x=1#v2', 'https://example.test/page')
    ).toBe('https://example.test/icon.svg');

    const usedNames = new Set<string>();
    const first = extractor.buildIconName(
    ...
    const second = extractor.buildIconName(
      'https://cdn.example.test/assets/logo.png',
      'png',
      'deadbeef',
      usedNames,
  ...
  it('collects and captures icon assets across DOM, manifest, and CSS sources', async () => {
    ...
    await page.route('**/*', async (route) => {
      ...
      if (
        pathname === '/logo.png' ||
        pathname === '/css-bg.png' ||
        pathname === '/inline-bg.png' ||
        pathname === '/imported-bg.png'
      ) {
        await route.fulfill({
          status: 200,
          body: `${pathname.slice(1)}-icon`,
    ...
    const iconAssets = await extractor.harvestIconsForRoute(
      snapshotId,
      page,
      snapshotBase.toString(),
      'all',
      accumulator
    );
    await context.close();
    ...
    const manifest = await storage.readJsonFile<{ icons: Array<{ localPath: string }> }>(
      snapshotId,
