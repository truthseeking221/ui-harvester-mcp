...
describe("storage utilities", () => {
  ...
  const nextRoot = () => path.join(os.tmpdir(), `ui-harvester-storage-${Date.now()}-${Math.round(Math.random() * 1_000_000)}`);
  ...
  it("builds normalized snapshot and screenshot paths", () => {
    ...
    expect(storage.screenshotPath("abc", "Hero Shot")).toBe(path.join(root, "snapshots", "abc", "evidence", "screenshots", "hero_shot.png"));
    expect(storage.buildViewportKey({ name: "Desktop", width: 1280, height: 720, deviceScaleFactor: 2 })).toBe("desktop_1280x720_dpr2");
    expect(storage.captureId("https://example.test/account/settings", { name: "Desktop", width: 1280, height: 720 }, "dark", "default")).toBe(
      "account_settings_desktop_1280x720_dark_default",
    );
    expect(storage.captureId("https://example.test/account/settings", { name: "Desktop", width: 1280, height: 720 }, "light", "default", "hover")).toBe(
      "account_settings_desktop_1280x720_light_default_hover",
    );
  });
  ...
  it("stores and loads typed JSON snapshots without schema coupling", async () => {
    ...
    const loaded = await storage.readJsonFile<{ nested: { route: string; width: number }; tags: string[] }>(snapshotId, "meta/inspect.json");
