...
async function resolveTargetLocator(page: Page, sourceState?: ValidationStateCapture): Promise<LocatorCandidate | null> {
  const meta = sourceState?.stateTargetMeta;
  const candidates: LocatorCandidate[] = [];

  if (sourceState?.stateTargetResolved?.locator) {
    candidates.push({ strategy: "resolved-locator", type: "selector", selector: sourceState.stateTargetResolved.locator, confidence: 1 });
  }
  if (meta?.locator) candidates.push({ strategy: "locator", type: "selector", selector: meta.locator, confidence: 0.99 });
  if (meta?.selector) candidates.push({ strategy: "selector", type: "selector", selector: meta.selector, confidence: 0.95 });
  if (meta?.id) candidates.push({ strategy: "id", type: "selector", selector: `#${escapeCssIdent(meta.id)}`, confidence: 0.94 });
  if (meta?.role) {
    candidates.push({
      strategy: "role",
      type: "selector",
  ...
  if (sourceState?.stateTarget) {
    candidates.push({ strategy: "legacy-target", type: "selector", selector: sourceState.stateTarget, confidence: 0.62 });
  }
  ...
  if (!candidates.length) {
    candidates.push({
      strategy: "generic",
      type: "selector",
      selector: "a,button,input,textarea,select,[role='button'],[role='link'],[role='tab'],[role='menuitem'],[role='combobox'],[role='listbox'],[role='checkbox'],[role='radio'],[role='switch'],summary",
      confidence: 0.3,
    });
  }
  ...
  switch (state) {
    ...
    case "open":
      if (stateInfo && (stateInfo.ariaExpanded !== null || stateInfo.tag === "summary")) {
        ...
        const afterExpanded = await target.evaluate((element) => element.getAttribute("aria-expanded")).catch(() => null);
    ...
    case "error":
      const probe = await target
        .evaluate(
          (element, desiredState) => {
            if (desiredState === "loading") {
              const loadingCandidates = [
                element,
                element.closest("[role='status'], [role='progressbar'], [aria-live]"),
                element.parentElement,
                ...Array.from(element.querySelectorAll?.("[aria-busy], [data-loading], [data-busy], [data-state], [data-status], [role='progressbar'], [aria-live], .loading, .spinner, .skeleton, .progress, .busy") || []),
              ].filter(Boolean) as Element[];
              const signalText = loadingCandidates
                .map((candidate) =>
                  `${candidate.getAttribute("aria-busy") || ""} ${candidate.getAttribute("data-loading") || ""} ${candidate.getAttribute("data-state") || ""} ${candidate.getAttribute(
                    "data-status",
                  ) || ""} ${candidate.className || ""}`,
                )
                .join(" | ")
            ...
            const errorCandidates = [
              ...
              element.parentElement,
              ...Array.from(element.querySelectorAll?.("[role='alert'], [role='status'], [aria-live], .error, .invalid, .danger, .warn") || []),
            ].filter(Boolean) as Element[];
            const stateText = errorCandidates
              .map((candidate) => `${candidate.getAttribute("data-error") || ""} ${candidate.getAttribute("aria-invalid") || ""} ${candidate.className || ""} ${candidate.getAttribute("data-state") || ""}`)
              .join(" | ")
              .toLowerCase();
...
function normalizeForPixelDiff(sourceImage: PNG, targetImage: PNG): { source: PNG; target: PNG; drifted: boolean } {
  ...
  if (width === sourceImage.width && height === sourceImage.height && width === targetImage.width && height === targetImage.height) {
...
export async function validateVisualMatch(args: ValidateInput): Promise<ValidationReport> {
  ...
  if (!routesToCheck.length) {
    ...
    for (const route of uniqueRequestedRoutes) {
      const routeMatches = viewportFiltered.filter((page) => isRouteMatch(route, page.route) || isRouteMatch(page.route, route));
    ...
    for (const sourcePage of sourcePages) {
      ...
      for (const state of requestedStates) {
        ...
        if (!targetResult.buffer.length || targetResult.failureReason) {
          const failureReason = targetResult.failureReason || (useFallbackState ? "state_capture_fallback" : "target_interaction_not_reproducible");
          reportItems.push({
            route: sourcePage.route,
            viewport: viewportLabel,
            state,
        ...
        const diffPercent = Number(((mismatches / (normalized.source.width * normalized.source.height)) * 100).toFixed(2));
        ...
        reportItems.push({
          ...
          sourceViewport: sourceCapture.stateTargetMeta?.viewport || `${sourcePage.viewport.width}x${sourcePage.viewport.height}`,
