...
const interactionStateSchema = z.enum(["default", "hover", "focus", "active", "checked", "selected", "disabled", "open", "loading", "error"] as const);
...
server.tool(
  "list_snapshots",
  "List all snapshots already extracted.",
  {},
  async () => {
    try {
...
server.tool(
  ...
  async ({ snapshotId, format, targetStack }) => {
    try {
      ...
      return { content: [{ type: "text", text: output }, { type: "text", text: `Wrote: ${file}` }] };
    } catch (error) {
      return toolError(error);
    }
  },
...
server.tool(
  ...
  async ({ snapshotId, targetStack, includeFigmaBlueprint, includeValidationTemplate }) => {
    ...
    const artifacts = buildUniversalPackageArtifacts(snapshot, stack).filter((entry) => {
  ...
  async ({ snapshotId, projectPath, targetStack, createFilesOnly }) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: JSON.stringify({ status: "ok", projectPath: base, created, note: "Generated scaffold files only; component refactor still requires manual mapping." }, null, 2),
        },
      ],
    };
    } catch (error) {
...
server.resource(
  ...
  async (uri, vars) => {
    ...
    const snapshot = await readSnapshot(snapshotId);
    return {
      contents: [{ uri: uri.href, mimeType: "application/json", text: JSON.stringify(snapshot.tokens.semantic, null, 2) }],
    };
  },
);
...
server.resource(
  ...
  async (uri, vars) => {
    ...
    if (archetype && archetype !== "all") {
      ...
      if (!recipe) {
        return {
          contents: [{ uri: uri.href, mimeType: "application/json", text: JSON.stringify({ error: "archetype not found" }, null, 2) }],
        };
      }
      return { contents: [{ uri: uri.href, mimeType: "application/json", text: JSON.stringify(recipe, null, 2) }] };
    }
    return { contents: [{ uri: uri.href, mimeType: "application/json", text: JSON.stringify(snapshot.components.inventory, null, 2) }] };
  },
);
...
server.resource(
  ...
  async (uri, vars) => {
    const snapshotId = String(vars.snapshotId ?? "");
    const snapshot = await readSnapshot(snapshotId);
    return { contents: [{ uri: uri.href, mimeType: "application/json", text: JSON.stringify(snapshot.validation, null, 2) }] };
  },
);
  ...
  async (uri, vars) => {
    ...
    if (artifactKey === "manifest") {
      try {
        ...
        return { contents: [{ uri: uri.href, mimeType: "application/json", text: JSON.stringify({ warning: "No universal package has been exported yet." }, null, 2) }] };
      }
    }
    ...
    try {
      ...
      return { contents: [{ uri: uri.href, mimeType: "application/json", text: JSON.stringify({ warning: `artifact missing: ${artifactKey}` }, null, 2) }] };
    }
    ...
    messages: [
      {
        ...
        content: {
          type: "text",
          text: `Use snapshot ${snapshotId} and target stack ${targetStack || "vite-tailwind"}.\n`
    ...
    messages: [
      {
        ...
        content: {
          type: "text",
          text: `You are applying design packages from ${snapshotId}.\n`
