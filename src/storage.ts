...
function parseViewportString(viewport: string): string {
  return viewport.toString().replace(/[^a-z0-9]/gi, "_").replace(/_+/g, "_");
}
...
export async function saveSnapshot(snapshot: UiSnapshotManifest): Promise<string> {
  ...
  await fs.writeFile(path.join(baseDir, "tokens", "semantic.json"), JSON.stringify(snapshot.tokens.semantic, null, 2), "utf8");
  await fs.writeFile(path.join(baseDir, "tokens", "breakpoints.json"), JSON.stringify(snapshot.tokens.breakpoints, null, 2), "utf8");
  await fs.writeFile(path.join(baseDir, "components", "inventory.json"), JSON.stringify(snapshot.components.inventory, null, 2), "utf8");
  await fs.writeFile(path.join(baseDir, "validation", "report.json"), JSON.stringify(snapshot.validation, null, 2), "utf8");
...
export async function listSnapshots(): Promise<string[]> {
  ...
  return files.filter((entry) => !entry.startsWith(".")).sort().reverse();
}
...
export async function writeBinaryFile(snapshotId: string, relativePath: string, data: Buffer | Uint8Array | string): Promise<string> {
  const baseDir = await ensureSnapshot(snapshotId);
...
export function buildViewportKey(viewport: { name: string; width: number; height: number; deviceScaleFactor?: number }) {
