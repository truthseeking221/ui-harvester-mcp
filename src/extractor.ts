...
function parseCanonicalLengthToNumber(input: string): number | null {
  ...
  if (token.startsWith('calc(') || token.startsWith('max(') || token.startsWith('min(') || token.startsWith('clamp('))
    return null;
...
function roundLengthForProfile(value: number | null, profile: CleaningProfile): number | null {
  ...
  return profileRound(
    value,
    PROFILE_LIMITS[profile]?.sizeValueDigits ?? PROFILE_LIMITS[DEFAULT_CLEANING_PROFILE].sizeValueDigits
  );
}
...
function canonicalizeStateTargetMeta(meta?: StateCapture['stateTargetMeta']): string[] {
  if (!meta) return [];
...
function isLikelyIconUrl(rawUrl: string, mime: string | null, context: IconContext, rel?: string): boolean {
  ...
  if (context === 'css-inline' || context === 'css-stylesheet') {
    return (
      ...
      candidate.includes('favicon') ||
      candidate.includes('apple-touch')
    );
  }
  const normalizedRel = (rel || '').toLowerCase();
  if (
    normalizedRel.includes('icon') ||
    normalizedRel.includes('apple-touch') ||
    normalizedRel.includes('msapplication')
  )
    return true;
  ...
  const bundle = await page.evaluate(
    (payload) => {
      const shouldCollect = (context: string) => {
        ...
        if (payload.profile === 'selected')
          return context === 'dom-link' || context === 'manifest' || context === 'img';
        return true;
      };
      ...
      const add = (
        sourceUrl: string,
        context: 'dom-link' | 'manifest' | 'img' | 'css-inline' | 'css-stylesheet',
        details: { rel?: string; nameHint?: string; inlineSvg?: string; width?: number; height?: number }
      ) => {
      ...
      document
        .querySelectorAll('source[src], source[srcset]')
        .forEach((source) => collectImgLike(source as HTMLSourceElement));
...
async function collectStylesheetIcons(
  page: Page,
...
async function downloadIconData(
  candidate: RawIconCandidate,
  page: Page,
  profileStats: IconHarvestStats
): Promise<{
  success: boolean;
  bytes: number | null;
  sha256: string | null;
...
export async function harvestIconsForRoute(
  ...
  const manifestEntries = await Promise.all(
    manifestCandidates.map((entry) => collectManifestIcons(page, entry.sourceUrl, sourcePage, iconCaptureProfile))
  );
  const stylesheetEntries = await Promise.all(
    stylesheetCandidates.map((entry) => collectStylesheetIcons(page, entry, sourcePage, iconCaptureProfile))
  );
  ...
  for (let index = 0; index < expanded.length; index += 1) {
    ...
    const extHint =
      extractExtFromSource(candidateUrl) ||
      extractExtFromSource(candidate.sourceUrl) ||
      (candidate.inlineSvg ? 'svg' : null);
    ...
    if (!likelyIconUrl && !sourceHint.startsWith('data:image/')) {
      ...
      const statusRecord = buildRecord(
        'skipped',
        `assets/icons/${safeIconFileName(fallbackNameHint)}.${ext}`,
        `${safeIconFileName(fallbackNameHint)}.${ext}`,
        ext,
        null,
        0,
        0,
    ...
    if (accumulator.bySourceUrl.has(recordSourceUrl)) {
      ...
      accumulator.stats.skipped += 1;
      const existingFromSameUrl = accumulator.icons.find(
        (entry) => entry.sourceUrl === recordSourceUrl && entry.status === 'captured'
      );
...
function normalizeCssLengthForSignature(
  raw: string,
...
function normalizePx(value: string, profile: CleaningProfile = DEFAULT_CLEANING_PROFILE): string {
  ...
  if (
    canonical.startsWith('calc(') ||
    canonical.startsWith('max(') ||
    canonical.startsWith('min(') ||
    canonical.startsWith('clamp(')
  ) {
    return canonical;
  }
  ...
  if (unit === 'cqw' || unit === 'cqh' || unit === 'cqi' || unit === 'cqb' || unit === 'cqmin' || unit === 'cqmax') {
    return profile === 'minimal'
      ? `${safeToFixed(numeric, digits)}${unit}`
      : `${safeToFixed(numeric, digits)}${unit};alias=${alias || normalizedLower}`;
  }
...
function normalizeStyleValueForSignature(
  ...
  if (!val) return '';
  if (key === 'color' || key === 'backgroundColor' || key === 'borderColor' || key === 'outlineColor')
    return normalizeColorValue(val, profile);
  ...
  const normalizedLayers = list.map((layer) => {
    ...
    const colorToken = tokens.find((token) => {
      ...
      if (
        ...
        token.startsWith('hwb') ||
        token.startsWith('#') ||
        NAMED_COLOR_HEX.has(token) ||
        token.startsWith('var(') ||
        token === 'transparent'
      ) {
        return true;
    ...
    const numeric = tokens
      ...
      .map((token) => normalizeShadowToken(token, profile))
      .filter(Boolean)
      .join(' ');
...
function normalizeRadiusValue(value: string, _profile: CleaningProfile): string {
  ...
  const parts = canonical
    .split(/[\s/]+/)
    .map((part) => normalizeCssLengthForSignature(part))
    .filter(Boolean) as string[];
...
export function normalizeColorValue(input: string, profile: CleaningProfile = DEFAULT_CLEANING_PROFILE): string {
  ...
  if (value.startsWith('url(') || value.startsWith('var(') || value.startsWith('env('))
    return profile === 'minimal' ? value : '';
  ...
  if (value.startsWith('rgb(') || value.startsWith('rgba(')) {
    ...
    const parts = [...head.replace(/,/g, ' ').split(/\s+/).filter(Boolean), ...(tail ? [tail.trim()] : [])].filter(
      Boolean
    );
...
function signatureFromStyles(style: NodeComputedStyle, profile: CleaningProfile = DEFAULT_CLEANING_PROFILE) {
  ...
  const customEntries = Object.keys(style.customProperties || {})
    ...
    .map((name) => {
      ...
      if (!raw) return '';
      const value =
        raw.startsWith('#') || raw.startsWith('var(') || raw.includes('calc(')
...
function toCssNodeStyle(
  raw: Record<string, string>,
  profile: CleaningProfile = DEFAULT_CLEANING_PROFILE
): NodeComputedStyle {
  const normalize = (key: keyof NodeComputedStyle) =>
    normalizeStyleValueForSignature(key, (raw?.[key] as string) || '', profile);
  return {
...
function classifyArchetype(sample: {
  ...
  const hasRadius = Boolean(
    sample.styles?.borderRadius && sample.styles.borderRadius !== '0' && sample.styles.borderRadius !== '0px'
  );
  const hasBorder = Boolean(
    sample.styles?.borderWidth && sample.styles.borderWidth !== '0' && sample.styles.borderWidth !== 'none'
  );
...
function styleFingerprintFromSamples(
  ...
  const scope = [route, theme, viewportName, viewport, depth].filter(Boolean).join('|');
  const scopeHint = `${scope}|samples=${samples.length}|visible=${visibleSampleCount}|depth=${depth}`;
  return createHash('sha1')
    .update(scopeHint ? `${scopeHint}|${signature}` : signature)
    .digest('hex');
}
...
async function gatherSamples(
  ...
  let dropped = 0;
  const areaThreshold = Number.isFinite(profileContext.sampleNoiseAreaThreshold)
    ? profileContext.sampleNoiseAreaThreshold
    : 0;
...
async function resolveStyleSignatureByLocator(
  ...
  const ordered = [
    ...new Set([
      ...
      ...(candidates || []),
      target.target.tag ? target.target.tag : '',
      '',
    ]),
  ];
  ...
  if (!target.found && target.target.tag && target.target.bbox) {
    const style = await page
      .evaluate(
        ...
        `bbox:${Math.round(target.target.bbox.x)},${Math.round(target.target.bbox.y)}`
      )
      .catch(() => null);
...
async function resolveInteractiveTarget(
  ...
  if (target.classTokens[0])
    strategyTryOrder.push({ strategy: 'class', selector: `.${CSS.escape(target.classTokens[0])}` });
  if (cleanText) strategyTryOrder.push({ strategy: 'text', selector: cleanText, text: cleanText });
  if (cleanAriaLabel)
    strategyTryOrder.push({ strategy: 'aria', selector: `[aria-label='${CSS.escape(target.ariaLabel || '')}']` });
  if (target.bbox)
    strategyTryOrder.push({
      strategy: 'bbox',
      selector: `bbox:${Math.round(target.bbox.x)},${Math.round(target.bbox.y)}`,
    });
  strategyTryOrder.push({
    strategy: 'role-text',
    selector: '',
    text: cleanText || '',
    filter: cleanRole ? `[role='${target.role}']` : '',
  });
  ...
  const normalizeSelector = (item: { strategy: string; selector: string; text?: string }) => {
    ...
    if (item.strategy === 'text' || item.strategy === 'role-text')
      return `${item.strategy}:${(item.text || '').slice(0, 120)}`;
    if (item.strategy === 'class' && item.selector.startsWith('.') && !item.selector.includes(' '))
      return `class=${item.selector}`;
    ...
    if (item.strategy === 'bbox' && target.bbox) {
      const selector = await page
        .evaluate((input) => {
          ...
          const query =
            "a,button,input,textarea,select,summary,[role='button'],[role='link'],[role='tab'],[role='menuitem'],[role='combobox'],[role='listbox'],[role='checkbox'],[role='radio'],[role='switch'],[role='list'],[role='navigation'],[role='menu'],[role='textbox']";
...
async function applyInteractionState(
  ...
  const controlState = await locator
    .evaluate((node) => {
      ...
      const isDisabled =
        ...
        element.getAttribute('aria-disabled') === '1';
      const isChecked =
        'checked' in (element as { checked?: boolean }) ? Boolean((element as { checked?: boolean }).checked) : false;
  ...
  const isCheckLike =
    ['checkbox', 'radio', 'switch'].includes(controlState.type) ||
    ['checkbox', 'radio', 'switch'].includes(controlState.role);
  const supportsChecked = isCheckLike;
  const supportsSelected = ['tab', 'option', 'menuitem', 'row', 'checkbox', 'radio', 'switch'].includes(
    controlState.role
  );
  const hasOpenTarget =
    controlState.hasOpenIndicator ||
    controlState.hasExpanded ||
    controlState.hasAriaControlHint ||
    controlState.hasDataOpenHint;
      ...
      const disabledProbe = await locator
        .evaluate(
          (node, payload: { canNative: boolean; canAria: boolean }) => {
            ...
            const ariaDisabled =
              payload.canAria && node instanceof Element
                ? node.getAttribute('aria-disabled') === 'true' || node.getAttribute('aria-disabled') === '1'
                : false;
            ...
            const hasDisabledSignal = Boolean(
              hasDisabled || ariaDisabled || disabledClass || dataState === 'disabled' || dataDisabled === 'true'
            );
            return {
              hasDisabledSignal,
              hasDisabled,
        ...
        try {
          await locator.evaluate(
            (node, payload) => {
              ...
              if (payload.hadPointerEvents == null || payload.hadPointerEvents === '')
      ...
      const loadingProbe = await locator
        .evaluate((node) => {
          const candidates = [
            node,
            node.closest(
              "button, a, summary, [role='button'], [role='tab'], [role='menuitem'], [role='switch'], [role='checkbox'], [role='radio']"
            ),
          ...
          const nearby = Array.from(
            ...
            .map(
              (candidate) =>
                `${candidate.className || ''} ${candidate.getAttribute('aria-busy') || ''} ${candidate.getAttribute('data-state') || ''}`
            )
            .join(' | ');
    ...
    case 'error': {
      ...
      const errorProbe = await locator
        .evaluate((node) => {
          ...
          const hasIndicator = /(error|invalid|danger|warn|failed|aria-invalid|data-error)/i.test(
            `${stateText} ${nearbyText}`
          );
    ...
    const normalizedSizeScale = sizeScale
      .filter(
        (entry, index, all) =>
          all.findIndex(
            (item) =>
              `${item.variant}|${item.minHeight}|${item.horizontalPadding}|${item.verticalPadding}` ===
              `${entry.variant}|${entry.minHeight}|${entry.horizontalPadding}|${entry.verticalPadding}`
          ) === index
      )
      .sort((a, b) => {
    ...
    for (const stateCapture of stateByArchetype[archetype] || []) {
      ...
      const cleanedState: ComponentVariantState = {
        ...
        provenance: [
          ...
          stateCapture.changedProperties?.length
            ? `changed:${(stateCapture.changedProperties || []).sort().join(',')}`
            : '',
        ].filter(Boolean),
      };
      ...
      if (dedupeStates.has(stateKey)) {
        ...
        if (existing) {
          existing.changedProperties = [
            ...new Set([...(existing.changedProperties || []), ...(cleanedState.changedProperties || [])]),
          ].sort();
          existing.changedPropertiesAdded = [
            ...new Set([...(existing.changedPropertiesAdded || []), ...(cleanedState.changedPropertiesAdded || [])]),
          ].sort();
          existing.changedPropertiesRemoved = [
            ...new Set([
              ...(existing.changedPropertiesRemoved || []),
              ...(cleanedState.changedPropertiesRemoved || []),
            ]),
          ].sort();
...
async function makeRouteCapture(
  ...
  const shouldThrottleStateCapture =
    !settling.stable && settling.unstableFrames > Math.max(2, Math.floor(waitConfig.mutationStabilityFrames * 0.6));
  const stateCaptureBudget = Math.max(
    1,
    settling.stable ? interactionBudget : Math.max(1, Math.floor(interactionBudget * 0.6))
  );
  ...
  const gathered = await gatherSamples(page, profileAdjustedSamples, profileAdjustedStride, cleaningContext, {
    route,
  ...
  const stateSequence = SUPPORTED_INTERACTION_STATES.slice(
    0,
    Math.max(0, shouldThrottleStateCapture ? 0 : stateBudget)
  );
  ...
  for (const state of stateSequence) {
    for (const target of capped) {
      ...
      const before = resolved.found
        ? await resolveStyleSignatureByLocator(page, resolved, cleaningContext.profile).catch(() => null)
        : null;
      const interaction = await applyInteractionState(page, state, resolved).catch((error) => {
        stateWarnings.push(
          `interaction_error route=${route} state=${state} target=${resolved.selector} ${String(error)}`
        );
        return {
      ...
      if (interaction?.skipped) {
        stateWarnings.push(
          `interaction_skipped route=${route} state=${state} target=${resolved.selector} ${interaction?.warnings[0] || ''}`.trim()
        );
      ...
      const shotSucceeded = await page
...
export function aggregateCoreTokens(
  ...
  const finalizeNumericBucket = (input: NumericObservation[], fallbackLimit: number) => {
    const trimmed =
      profile === 'minimal' ? { kept: input, removed: 0 } : outlierTrimmed(input, cleaningContext.outlierIqr);
...
export function toComponentInventory(
  ...
  for (const page of pages) {
    for (const recipe of page.componentInventory) {
      ...
      const sizeScaleKey = (entry: ComponentRecipe['sizeScale'][number]) =>
        `${entry.variant}|${entry.minHeight}|${entry.horizontalPadding}|${entry.verticalPadding}`;
      ...
      const canonicalSizeScale = recipe.sizeScale
        .map((entry) => ({
          ...
          minHeight: entry.minHeight
            ? toCanonicalLengthToken(
                parseLengthForProfile(entry.minHeight, profileContext.profile) ?? NaN,
                profileContext.profile
              )
            : null,
          horizontalPadding: entry.horizontalPadding
            ? toCanonicalLengthToken(
                parseLengthForProfile(entry.horizontalPadding, profileContext.profile) ?? NaN,
                profileContext.profile
              )
            : null,
          verticalPadding: entry.verticalPadding
            ? toCanonicalLengthToken(
                parseLengthForProfile(entry.verticalPadding, profileContext.profile) ?? NaN,
                profileContext.profile
              )
            : null,
        }))
      ...
      for (const state of recipe.states) {
        ...
        existing.changedProperties = [
          ...new Set([...(existing.changedProperties || []), ...(state.changedProperties || [])]),
        ].sort();
        existing.changedPropertiesAdded = [
          ...new Set([...(existing.changedPropertiesAdded || []), ...(state.changedPropertiesAdded || [])]),
        ].sort();
        existing.changedPropertiesRemoved = [
          ...new Set([...(existing.changedPropertiesRemoved || []), ...(state.changedPropertiesRemoved || [])]),
        ].sort();
        existing.examples = [...new Set([...(existing.examples || []), ...(state.examples || [])])].slice(
          0,
          maxExamples
        );
...
async function discoverSameOriginLinks(
  page: Page,
  sourceUrl: string,
  routeDepth: number,
  maxDepth: number
  ...
  for (const href of hrefs) {
    try {
      ...
      if (
        ...
        resolved.pathname.endsWith('.jpeg') ||
        resolved.pathname.endsWith('.webp')
      )
        continue;
          ...
          try {
            ...
            if (
              seenFingerprints.has(
                capture.routeFingerprint ||
                  `${capture.layoutFingerprint}-${route}-${viewport.width}x${viewport.height}-${theme}`
              )
            ) {
              ...
              routeSummary.warnings?.push(
                `duplicate-fingerprint route=${route} theme=${theme} viewport=${viewport.name}`
              );
              continue;
            }
            ...
            if (captureMs > DEFAULT_WAIT.networkQuietMs * 3 + DEFAULT_WAIT.settleMs + 200) {
              routeSummary.warnings?.push(
                `long_capture_ms route=${route} theme=${theme} viewport=${viewport.name} ms=${captureMs}`
              );
            }

            const capturedIcons = await harvestIconsForRoute(
              snapshotId,
              page,
              routeBase,
              iconCaptureProfile,
            ...
            routeSummary.warnings?.push(
              `capture-failed route=${route} theme=${theme} viewport=${viewport.name} ${String(error)}`
            );
          } finally {
            await context.close();
          }
